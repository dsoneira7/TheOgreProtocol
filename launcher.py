...
ports = random.sample(port_range, num_relays)
exit_port = "6666"
for port in ports[:num_relays]:
    os.system("python node.py " + args.dir_auth_ip + " " + str(port) + " " + args.dir_auth_ip + " " + str(
        args.dir_auth_port) + " &")
    time.sleep(1)

os.system("python client.py " + " " + args.dir_auth_ip + " " + str(args.dir_auth_port))  # + " &")
